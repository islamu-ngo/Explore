...
| Standalone / Combined | One API/BFF process at `https://localhost:7180` | `Event.Standalone` invokes API startup before Combined Blazor initialization and owns workers, migrations/seeding gates, shared readiness/health, and shutdown once. Combined omits YARP and remote-API readiness. |
