...
5. if the setup page reports `Environment`, re-enter the configured `SETUP_SECRET`. If it reports `Generated`, configure `SETUP_SECRET` and restart the API because the internal random fallback has no readback path. Raw values are never shown in startup output.
