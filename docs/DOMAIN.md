...
Payment attempts, Stripe hosted Checkout/direct charges, payment webhooks and reconciliation, capture, refunds, disputes, admission, QR/check-in, transfers, payouts, and legal/tax/invoice support remain Phase 18-or-later work and are not implemented domain behavior. A positive order may reach `AwaitingPayment`, but Phase 17 creates no provider Checkout session and no payment success evidence. `ProtectedDelayedPayout` remains approval-gated and absent from the default profile.
