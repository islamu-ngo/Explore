...
Paid-event commerce and admission are approved architecture but not yet implemented runtime state. ADR-022 requires actor-bound Stripe Connect `OrganizerDirect`, immutable merchant/currency/policy snapshots, local reservation-aware promotions, independent `PaymentAttempt` and `RefundAttempt` state, and post-commit provider reconciliation. ADR-023 requires one independently revocable `AdmissionTicket` per concrete ticket assignment, opaque rotatable QR credentials, entitlement-targeted append-only check-in/undo, scoped scanner capabilities, transfer rotation, and anti-enumeration recovery. ADR-024 keeps waitlists and event-bound add-ons inside Event while marketing, accounting, tax determination, and legal invoice/credit-note issuance remain external specialist domains. `ProtectedDelayedPayout` remains approval-gated and is not part of the default payment profile.
