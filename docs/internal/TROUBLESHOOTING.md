...
- OAuth state and handoffs use the primary database through the private API. Restore API/database access and the existing OAuth signing authority after an outage; do not retry a consume whose response was lost. Start a new login from the handle form. The current `atproto-authentication` check is still passive and cannot certify an operational store round trip. Persisted encrypted OAuth sessions remain separate from transient state.
