...
31. **Secrets Source of Truth (Infisical or `.env` only)**: Secrets, passwords, API tokens, connection strings, and encryption keys must never be hard-coded or defined in `Explore.AppHost` (`AppHost.cs`), test fixtures, controllers, or code. Secrets reside strictly in **Infisical** or **`.env`** (with template keys documented in **`.env.example`**). Tests and local hosting bind secrets dynamically via environment variables or secret provider mocks.
