...
- Authenticated AI assistant API foundation is available under `/api/ai/assistant`. It exposes bootstrap, private conversation list/detail/create, send-message with `Idempotency-Key`, and run-status routes. Conversation affordances are HAL-driven (`self`, `collection`, active-state `send-message`, and collection `create`); provider errors map to safe ProblemDetails and tool calls remain proposed actions only.
