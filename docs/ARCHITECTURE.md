...
Handwritten immutable contracts follow the [canonical record-selection policy](GOVERNANCE.md#canonical-record-selection-policy). Concrete Application MediatR requests, immutable DTO/payload snapshots, and valid-state command results use record semantics; EF entities, persisted outbox lifecycle rows, generated NSwag contracts, and Blazor edit/component state remain classes. This is a shallow immutability boundary: every published collection-bearing record exposes a read-only/immutable shape and copies mutable input, while record equality does not make sequence contents structurally equal or imply thread safety. `PublishedCollectionContractArchitectureTests` discovers this contract across Domain, Application, API, and Blazor and requires exact reasoned dispositions for any mutable exception.
