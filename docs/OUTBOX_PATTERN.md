...
`PdsSyncOutbox` preserves the database-first federation boundary. A successful local lifecycle transition and its immutable delivery row commit atomically; mapping or local validation failure creates no remote work. `PdsSyncWorker` claims rows with expiring, fenced leases and rechecks effective capability, self-consent, linked OAuth session, source version, and public-location disclosure immediately before CarpaNet PDS I/O. Stable record keys and reconciliation make retries converge after uncertain settlement. Event URI/CID settles before dependent RSVP strong references become claimable, and the canonical `AtprotoRecord` is linked back to the already-committed local event. Remote failure never deletes or rolls back that event.
