...
ADR-022 approves Stripe Connect `OrganizerDirect` as the first paid-event profile. Task 16.3 adds only the self-hoster-owned instance/server-only secret definitions `payments.stripe.platform_secret_key` and `payments.stripe.webhook_secret`, mapped from Infisical `/stripe/STRIPE_PLATFORM_SECRET_KEY` and `/stripe/STRIPE_WEBHOOK_SECRET`; it adds no Stripe package, endpoint, migration, onboarding, webhook intake, reconciliation, or runtime payment capability. Each deployment must register and operate its own Stripe Connect platform; unrelated self-hosters never receive ISLAMU credentials or financial guarantees. Future checkout must disclose the independent instance operator and organizer merchant, use the organizer's connected-account direct-charge context, and keep provider calls outside Event transactions. Runtime Stripe I/O, onboarding, webhooks, and reconciliation are Task 16.4 work. Marketing, accounting, tax determination, and legal invoicing remain optional external integrations. `ProtectedDelayedPayout` is not escrow and stays unavailable without its separate Stripe, legal, Islamic-finance, and operator approvals.
