...
Each governed release line uses `docs/releases/<version>/release.yaml` plus `summary.md`. `release-context.v1.json` and `release-notes.md` are generated by `prepare` and must not be edited by hand.
...
`docs/releases/<version>` directories for unapproved releases.
...
- For the first governed release only, defined by no reachable governed stable SemVer
  ...
  `docs/releases/baselines/<ref>.v1.json` evidence must already record the exact full
...
The command writes `docs/releases/baselines/changelog-baseline-YYYY-MM-DD.v1.json`
