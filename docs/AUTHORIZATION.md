...
ATProto credential operations remain server-private. The bootstrap/current-session/delete bridge is excluded from API discovery and generated browser contracts; generic raw-token create/update routes do not exist. Public session reads return only the current user's safe metadata (`id`, `provider`, `pdsHost`, `expiresAt`), and deletion is authorized, self-scoped, and idempotent. Direct `AtprotoRecord` mutations are likewise absent: lifecycle-owned outboxes and canonical Jetstream ingress are the only record-write authorities. If a client renders a resource action, it must use the returned HAL relation rather than infer authority from the ATProto provider, DID, roles, or claims.
