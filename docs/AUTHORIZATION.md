...
Outgoing webhook management uses the `islamuevent_webhook` resource kind. The initial action catalog is namespaced as `webhook:view`, `webhook:create`, `webhook:update`, `webhook:delete`, `webhook:rotate-secret`, `webhook:test`, `webhook:retry`, `webhook:view-delivery`, `webhook:manage-provider`, and `webhook:open-provider-portal`. The Svix App Portal route uses `webhook:open-provider-portal` through the MediatR authorization pipeline, with tenant id and provider attributes supplied by `ISecureRequest`. Bundled Cerbos policy grants instance admins all webhook actions, tenant admins webhook actions within their tenant scope, and organization admins delegated organization-owned webhook actions when the resource carries `organizationId` and `allowOrganizationWebhooks=true`. Organization admins cannot use `webhook:manage-provider`. Local fallback mirrors that split: tenant admin authority remains tenant scoped, while org admin authority is limited to delegated org-owned webhook resources.
