...
Payment Checkout, payment-attempt cancellation, refund, dispute, and reconciliation actions remain Phase 18 or later and require dedicated resource decisions and matching HAL relations before implementation. Ticket, check-in, transfer, waitlist, and add-on actions likewise retain their owning phase boundaries. Scanner capabilities are separate, narrow, expiring authorities and do not grant roster, order, payment, refund, or attendee-answer access. `ProtectedDelayedPayout` cannot be authorized until its external approval evidence gate is current.
