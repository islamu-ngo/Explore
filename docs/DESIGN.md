...
### SetupAccessGateway
- Structure: `/setup` is a dedicated split-screen gateway inside `SetupLayout`, not an `OnboardingWorkspace` step. The branding panel and setup-secret form panel are siblings; the form owns the page's single `h1` and labeled password control.
- Geometry: desktop uses the established `58%` diagonal branding panel over a `50%` form panel. Component tokens name the panel sizes, diagonal cut, content limits, and gradient stops. At the `60em` breakpoint the panels stack; at `37.5em` the compact spacing and type projection applies without horizontal clipping.
- Styling: spacing, typography, backgrounds, text, and interaction colors resolve through `--isl-*` semantic tokens and MudBlazor theme variables. The diagonal clip is decorative; content order remains branding then form in both LTR and RTL.
- Security and routing: setup-secret validation persists only through the BFF-owned HttpOnly cookie flow. Detected Keycloak quick continue hard-navigates through login to the authoritative post-entry destination; absent Keycloak or explicit configuration hard-navigates to `/onboarding/auth-provider`. No setup secret, draft, token, or completion state enters browser storage.
- Accessibility: the form remains keyboard operable with visible labels and status/error text. Narrow layouts scroll vertically, preserve the theme/language controls from `SetupLayout`, and retain at least the global minimum target size.

### OnboardingWorkspace
- Reference contract: current onboarding screenshots and the unrelated prototype are context only. The workspace uses ISLAMU Event tokens, typography, wrappers, copy, and normal QA against this contract; it does not copy prototype branding, wording, draft semantics, or a fixed step count.
- Structure: display/navigation-only content inside `SetupLayout`'s existing `main#main-content`, never a nested `main` or a full-page card. The page's single `h1` starts the main column, followed immediately by the named progress `nav` and focused content `section`, with a persistent right `aside`, contextual help, and balanced footer actions. The existing setup theme and language controls dock at the top of the rail. The summary uses native `details`/`summary` only for the mobile menu.
- Data and authority: parents project localized labels, visible steps, server-authored statuses, safe revisit links, and actions already permitted by the appropriate HAL relation. The primitive does not call APIs, inspect roles or claims, derive provider/deployment/completion state, or persist browser workflow state.
- Progress: setup access is visually related but outside numbered progress. The default instance journey is Authentication, Authorization, Site profile, and Readiness/Launch. The visible numbered set is conditional; `Step n of m`, segmented progress, current label, and `aria-current="step"` are calculated only from that parent-projected set. Authentication, Authorization, and the combined profile/readiness route all project into this same workspace shell; post-authentication routing must never fall back to a standalone “Setup Overview” surface. The main progress strip shows only the current step name; other step names and visible ordinals belong in the setup-summary rail. Deployment-managed steps may be omitted or shown as configured, but route visits never make a step complete. Tenant onboarding is a separate journey.
- Geometry: at 1280px and above, use a full-width two-region grid with the right summary rail occupying one quarter of the page and the main workspace three quarters. From 768px through 1279px, the right rail occupies one third and the main workspace two thirds. The rail uses tonal surface separation rather than elevation, begins with theme and language controls, and keeps the numbered setup summary visible while the page scrolls. Below 768px, replace the rail with a sticky top navigation bar containing the existing theme/language controls and a menu-icon native disclosure for setup summary and help. At 375px, retain the current-step label and `n of m`, allow long labels to wrap, balance Exit and primary actions, and never create viewport-level horizontal overflow.
- States: use visible text plus iconography and semantic token treatment for loading, error, locked remediation, skipped/configured, complete, current, upcoming, and dirty states; color is supplementary. Blocking preflight prevents Launch, warnings remain visible and nonblocking unless the server requires acknowledgement, and retry re-fetches authoritative state. A completed authentication task may retain a server-authorized management action.
- Exit and resume: Save and exit exists only when the parent has an existing server-side save action. Otherwise dirty input requires explicit discard confirmation before exit. Never persist setup secrets, provider credentials, tokens, unsaved form fields, or browser completion state in local or session storage. `StartupGate`/`StartupRoutingService` resumes the earliest incomplete or failed-remediation step from authoritative state; setup-secret and OIDC transitions may hard-reload.
- Accessibility and resilience: use logical CSS properties, native buttons/links, `--isl-target-min`, visible focus, polite status updates, sequential headings, and stable help/footer locations. The contract supports LTR/RTL, light/dark themes, forced colors, `prefers-reduced-motion`, and long localized copy without clipping or relying on hover, color, animation, or client-only navigation.
