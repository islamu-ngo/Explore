...
- Progress: setup access is visually related but outside numbered progress. The default instance journey is Authentication, Site profile, Authorization, and Readiness/Launch. The visible numbered set is conditional; `Step n of m`, segmented progress, current label, and `aria-current="step"` are calculated only from that parent-projected set. The main progress strip shows only the current step name; other step names and visible ordinals belong in the setup-summary rail. Deployment-managed steps may be omitted or shown as configured, but route visits never make a step complete. Tenant onboarding is a separate journey.
