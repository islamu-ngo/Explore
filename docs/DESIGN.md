...
- Structure: display/navigation-only content inside `SetupLayout`'s existing `main#main-content`, never a nested `main` or a full-page card. It contains one workspace `header`, a named progress `nav`, one focused content `section` with the page's single `h1`, an optional complementary `aside`, contextual help, and a `footer` action row. The summary uses native `details`/`summary` when it becomes in-flow.
...
- Progress: setup access is visually related but outside numbered progress. The default instance journey is Authentication, Site profile, Authorization, and Readiness/Launch. The visible numbered set is conditional; `Step n of m`, segmented progress, current label, and `aria-current="step"` are calculated only from that parent-projected set. Deployment-managed steps may be omitted or shown as configured, but route visits never make a step complete. Tenant onboarding is a separate journey.
- Geometry: at 1280px and above, constrain the workspace to `80rem` and use a two-region grid with a flexible main column, a `20rem` to `24rem` summary rail, `--isl-space-8` gap, and tonal border/surface separation rather than elevation. At 768px, render focused content at full width and place a collapsed in-flow native summary directly after progress. At 375px, retain the current-step label and `n of m`, allow long labels to wrap, stack footer actions without covering the keyboard or validation, and never create viewport-level horizontal overflow.
