// ABOUTME: Unit tests for UpdateTenantMemberDtoValidator repository-backed validation.
// ABOUTME: Covers member ID, user lookup, and role lookup validation for tenant assignments.
...
public sealed class UpdateTenantMemberDtoValidatorTests
{
    private readonly IUserRepository _userRepository = Substitute.For<IUserRepository>();
    private readonly IRoleRepository _roleRepository = Substitute.For<IRoleRepository>();
    private readonly UpdateTenantMemberDtoValidator _validator;

    public UpdateTenantMemberDtoValidatorTests()
    {
        _validator = new UpdateTenantMemberDtoValidator(_userRepository, _roleRepository);
    }
    ...
    [Test]
    public async Task Validate_WhenIdIsEmpty_ReturnsRequiredError()
    {
        var dto = CreateValidDto();
        dto.Id = Guid.Empty;
        SetupExistingUserAndRole(dto);
        ...
        await Assert.That(result.Errors.Select(error => error.ErrorMessage))
            .Contains("Tenant Member ID is required");
    }
    ...
    private void SetupExistingUserAndRole(UpdateTenantMemberDto dto)
    {
        _userRepository.Exists(dto.UserId).Returns(true);
        _roleRepository.GetByIdAsync(dto.RoleId).Returns(CreateTenantRole(dto.RoleId));
    }
