...
| Accessibility, localization, RTL, and cultural fit | `Concern` | Strong design traceability exists: WCAG 2.2 AA target, skip links, landmarks, heading rules, live regions, focus management, logical CSS properties, Arabic/RTL support, localization providers, and culturally aware event metadata. `docs/ACCESSIBILITY_ARTIFACTS.md` explicitly says release-specific evidence is still a template and Playwright/axe-core scans are deferred, so this cannot be treated as a completed accessibility validation. |
