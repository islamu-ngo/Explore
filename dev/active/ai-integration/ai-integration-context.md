...
- Added host-backed Phase 3 AI API flow tests. `AiAssistantApiFlowTests` exercise the real ASP.NET Core host, auth middleware, routing, `AiAssistantController`, MediatR handlers, fake AI provider, fixed tenant AI settings, and HAL response serialization for no-anonymous history/list access, disabled-assistant 403 ProblemDetails, fake-provider create/send/detail flow, idempotency replay over HTTP, idempotency conflict through the existing API idempotency middleware, and cross-user detail absence.
- Fixed AI aggregate update semantics in `AiConversationRepository.Update`. When an `AiConversation` aggregate is already tracked, the repository now saves changes directly instead of forcing the whole aggregate graph to `Modified`, which avoids treating newly added messages/runs/actions as updates. Detached aggregates still use the generic repository update path.
- The host-backed API flow tests intentionally replace AI persistence and idempotency with focused in-memory test doubles inside the test host. This keeps the real controller/MediatR/auth/provider flow covered while avoiding the currently deferred EF AI migration and unrelated dirty EF/model/project-reference blockers.
...
- Phase 3 conversation foundation is in progress: safe DTOs, create/list/detail/run-status/send-message handlers, prompt builder/parser hardening, authenticated API routes, AI conversation HAL affordance policy, focused API controller/HAL contract tests, and host-backed AI API flow tests exist; provider-failure/true cross-tenant/DB-backed EF API tests, OpenAPI/client generation, abuse/rate limiting, and persistence integration tests are still pending.

### ⏭️ NEXT

1. Add remaining provider-failure, true cross-tenant, and DB-backed EF AI API flow tests after the migration/project-reference blockers are isolated.
...
| `Event.API.IntegrationTests/Features/AiAssistantApiFlowTests.cs` | New | API Tests | Host-backed AI assistant API flow validation. | Implemented with fixed AI settings, fake provider, and in-memory AI/idempotency repositories to avoid deferred EF migration blockers. |
...
- Host-backed AI API flow tests now cover no-anonymous list/history access, disabled assistant, fake-provider create/send/detail, idempotency replay, API middleware idempotency conflict, cross-user absence, and HAL links through a real test host. Provider-failure, true cross-tenant, and DB-backed EF AI persistence API tests remain pending until migration/project-reference blockers are isolated.
...
- **Current state:** Implementation has started. Phase 1 domain entities/statuses/tests are implemented. Phase 1 persistence code now includes EF DbSets/configurations/query filters and AI repository contract/implementation. Phase 2 internal provider/config contracts/settings, Infrastructure provider settings validation, deterministic fake provider, OpenAI-compatible HTTP adapter, runtime provider selector, authenticated bootstrap query/API, provider health, egress safety, and safe telemetry are implemented. Phase 3 conversation DTOs, create/list/detail/run-status/send-message Application handlers, prompt builder/parser hardening, authenticated API routes, AI conversation HAL policy, focused API controller/HAL contract tests, and host-backed AI API flow tests are implemented. Migration/schema docs/persistence tests are not implemented yet.
- **Next action:** Add remaining provider-failure, true cross-tenant, and DB-backed EF AI API flow tests after migration/project-reference blockers are isolated, or proceed to OpenAPI/generated client/changelog if the current host-backed coverage is sufficient for contract stabilization. Separately decide migration strategy in the dirty worktree before generating an AI-scoped migration, updating schema docs, and adding persistence integration tests.
