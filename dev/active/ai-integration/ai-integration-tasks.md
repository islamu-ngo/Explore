...
- **Completed:** Phase 0 approval/baseline/provider decision; Phase 1 domain statuses/entities/tests plus EF mappings/repository/schema docs/PostgreSQL-backed repository tests; Phase 2 provider-neutral Application contracts, expanded AI settings definitions/group, provider settings validator, deterministic fake provider, authenticated bootstrap query/API surface, provider health, egress safety, safe telemetry, OpenAI-compatible HTTP adapter, runtime provider selector; Phase 3 conversation DTO/query/create/detail/run-status/send-message/prompt-parser/API route/HAL policy/API contract-test/host-backed flow-test foundation; regenerated OpenAPI/client/changelog verification; AI authorization catalog/action parity foundation; MVP abuse/rate-limit/bounds gates.
- **Current priority:** Prepare for Blazor UI only after confirming remaining retention/redaction posture and product-surface gating.
- **Next recommended slice:** Re-read active event creation/event-scoped roles workstreams before Phase 5 `CreateEventDraft`, or add retention cleanup/redaction jobs if broad history exposure is planned first.
...
- [x] **1.6 Add EF migration and schema docs**
  - **Files:** `Explore.Persistence/Migrations/20260529173418_domainupdate.cs`, `Explore.Persistence/Migrations/20260531165911_domainupdate4.cs`, `ExploreDbContextModelSnapshot.cs`, `schemas/islamu-event.md`.
  - **Acceptance:** AI tables are present in the migrated model and schema docs list AI conversation/message/run/reference/proposed-action/tool-execution table purpose and relationships. Caveat: the AI schema landed in the earlier mixed `20260529173418_domainupdate.cs` migration, while the latest `20260531165911_domainupdate4.cs` is an empty snapshot refresh; this is not a cleanly scoped AI-only migration.
  - **Validation:** LSP diagnostics were clean for `schemas/islamu-event.md`. `dotnet build Explore.Persistence/Explore.Persistence.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false` passed. PostgreSQL-backed AI repository tests in task 1.7 applied the migrated schema successfully.
  - **Effort:** M
  - **Dependencies:** 1.4.

- [x] **1.7 Add AI persistence tests**
  - **Files:** `Event.Persistence.IntegrationTests/Repositories/AiConversationRepositoryTests.cs`.
  - **Acceptance:** PostgreSQL-backed tests cover create/update with tracked aggregate children, message ordering, run/action persistence, tenant-filtered recent lists, tenant-filtered detail lookups, tenant-scoped daily user-message quota counts, and tenant-filtered proposed-action lookup.
  - **Validation:** LSP diagnostics were clean for the AI repository and tests. `dotnet build Event.Persistence.IntegrationTests/Event.Persistence.IntegrationTests.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false` passed with existing non-AI warnings. `dotnet test --project Event.Persistence.IntegrationTests/Event.Persistence.IntegrationTests.csproj --configuration Release --no-build --verbosity quiet -- --treenode-filter "/*/*/*AiConversationRepositoryTests*/*" --minimum-expected-tests 5 --no-progress --maximum-parallel-tests 1` passed: 5 total, 0 failed. The repository update path now explicitly saves new aggregate child rows after sanitizing detached navigation references so EF does not reinsert the root conversation.
  - **Effort:** M
...
- [x] **3.6 Add conversation/API tests**
  ...
  - **Acceptance:** Host-backed API tests cover no anonymous conversation history/list access, disabled assistant 403 ProblemDetails, fake-provider create/send/detail flow, idempotency replay over HTTP, idempotency conflict through the existing API middleware (`idempotency_key_reuse`), provider failure returning safe 503 ProblemDetails, cross-user absence for private conversation detail, true tenant-header isolation for conversation detail, and HAL `send-message` links on real API responses. Tests use real ASP.NET Core host/routing/auth/controller/MediatR flow with fake/failing providers, fixed AI settings, and focused tenant-aware in-memory AI/idempotency repositories to avoid the currently deferred EF AI migration and unrelated dirty EF model blockers. DB-backed EF AI persistence API flow tests remain as follow-up coverage.
  - **Validation:** LSP diagnostics were clean for `AiAssistantApiFlowTests.cs` and the AI repository update. `dotnet build Explore.Persistence/Explore.Persistence.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false --no-dependencies` passed. `dotnet build Event.API.IntegrationTests/Event.API.IntegrationTests.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false -p:BuildProjectReferences=false` passed with existing non-AI warnings after the test-double and expectation updates. Targeted host-backed API flow tests passed: `dotnet test --project Event.API.IntegrationTests/Event.API.IntegrationTests.csproj --configuration Release --no-build --verbosity quiet -- --treenode-filter "/*/*/*AiAssistantApiFlowTests*/*" --minimum-expected-tests 8 --no-progress --maximum-parallel-tests 1` returned 8 total, 0 failed. Normal project-reference API integration build is still blocked by unrelated dirty Application storage-service compile errors.
  - **Effort:** L
  - **Dependencies:** 3.2-3.5.

- [x] **3.6b Add DB-backed EF AI persistence API flow tests**
  - **Files:** `Event.API.IntegrationTests/Features/AiAssistantDbBackedApiFlowTests.cs`, `Event.API.IntegrationTests/Fixtures/PostgreSqlApiWebApplicationFactory.cs`, `Event.API.IntegrationTests/Fixtures/PostgreSqlApiFixtureBase.cs`, `Explore.API/Middleware/IdempotencyMiddleware.cs`, `Explore.Persistence/Repositories/AiConversationRepository.cs`.
  - **Acceptance:** PostgreSQL-backed API tests use the real API host, real EF AI conversation repository, real idempotency repository, migrated AI tables, fixed fake AI settings, and fake provider to cover conversation create/send/detail, Application-owned AI idempotency replay/conflict over HTTP, and owner privacy. The PostgreSQL fixture now supports targeted service overrides. The global API idempotency middleware skips AI message-send routes so `SendAiMessageCommandHandler` remains the single source of truth for run-level idempotency. Remaining DB-backed gaps are provider-failure and true tenant-switch variants; host-backed tests already cover those behaviors with focused test doubles.
  - **Validation:** LSP diagnostics were clean for the DB-backed AI API flow test, PostgreSQL fixture hook, idempotency middleware, and AI repository. `dotnet build Explore.API/Explore.API.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false` passed with existing non-AI warnings. `dotnet build Event.API.IntegrationTests/Event.API.IntegrationTests.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false` passed with existing non-AI warnings. `dotnet test --project Event.API.IntegrationTests/Event.API.IntegrationTests.csproj --configuration Release --no-build --verbosity quiet -- --treenode-filter "/*/*/*AiAssistantDbBackedApiFlowTests*/*" --minimum-expected-tests 4 --no-progress --maximum-parallel-tests 1` passed: 4 total, 0 failed. The repository update path was fixed to handle detached post-provider aggregate updates through explicit root update plus child insert/update persistence.
  - **Effort:** M
  - **Dependencies:** 3.6, 1.6 for DB-backed EF path.

- [x] **3.7 Update OpenAPI/generated client/changelog**
  - **Files:** `schemas/openapi.json`, `Explore.Blazor.Client/Clients/EventApiClient.g.cs`, `docs/API_CHANGELOG.md`, maybe `docs/API.md`.
  - **Acceptance:** Generated AI methods are available, operation IDs are stable, and changelog documents the authenticated AI assistant API foundation and idempotency behavior. Generated client includes bootstrap, conversation list/create/detail, send-message with `Idempotency-Key`, and run-status methods.
  - **Validation:** `schemas/openapi.json` contains AI paths/operation IDs; `Explore.Blazor.Client/Clients/EventApiClient.g.cs` contains generated AI methods/DTOs; `docs/API_CHANGELOG.md` documents the AI assistant API foundation; `docs/API_CONTRACT_INVENTORY.md` lists AI routes as authenticated. `dotnet build Explore.Blazor.Client/Explore.Blazor.Client.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false` passed with one existing `ASPDEPR001` generated-client warning.
  - **Effort:** M
  - **Dependencies:** 3.4.

- [x] **3.8 Add AI authorization catalog and endpoint safety tests**
  ...
  - **Acceptance:** Added `islamuevent_ai_conversation` resource kind, `view`/`create`/`send_message` action constants, AI conversation resource descriptors/registry mappings, fallback local authorization, machine-scope mapping, Cerbos policy/schema, and `[AuthorizeResource]` metadata for AI create/list/detail/run-status/send requests. AI conversation HAL links now carry explicit permission metadata through `RequirePermission(...)` while handlers still enforce owner and tenant isolation. Private AI endpoints remain on an `[Authorize]`/`Authenticated` controller; no anonymous history/list access remains covered by existing host-backed API flow tests.
  - **Validation:** LSP diagnostics were clean for AI authorization catalogs, request metadata, HAL policy, fallback authorization, and tests. `dotnet build Explore.Application/Explore.Application.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false`, `dotnet build Explore.API/Explore.API.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false`, and `dotnet build Explore.Infrastructure/Explore.Infrastructure.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false` passed with existing non-AI warnings only. `dotnet test --project Event.Application.UnitTests/Event.Application.UnitTests.csproj --configuration Release --no-build --verbosity quiet -- --treenode-filter "/*/*/*AiAssistantAuthorizationMetadataTests*/*|/*/*/*MachineScopeMappingTests*/*" --minimum-expected-tests 2 --no-progress --maximum-parallel-tests 1` broadened to the full Application unit suite and passed: 1172 total, 0 failed. `Event.Architecture.Tests` authorization parity was attempted and still fails on pre-existing non-AI HATEOAS policies (`StorageObjectLinkPolicy`, `ActorLinkPolicy`, `EventLinkPolicy`) that use non-`AuthorizationActions` permission metadata; AI links use `AuthorizationActions.AiConversations.*`.
  - **Effort:** M
  - **Dependencies:** 3.4-3.5.

- [x] **3.9 Add MVP abuse, bounds, and retention gate**
  - **Files:** AI settings, rate limiting policy/API config, handlers/tests, `docs/CONFIGURATION.md`, `docs/OPERATIONS.md`.
  - **Acceptance:** Added tenant-governed per-user daily, per-tenant daily, per-user concurrent-run, and selected-reference bounds. Send orchestration now checks idempotency replay first, then tenant governance, model readiness, ownership/state, user daily quota, tenant daily quota, and concurrent-run quota before any provider call. Excess quota returns `BaseCommandResponse<Guid>.SetQuotaExceeded(...)`, which the API maps to safe ProblemDetails without prompts/provider content. The AI send route now has an `AiAssistant` API rate-limit policy. Existing prompt builder/parser gates still bound provider context, force non-streaming, and enforce model/tool allow-lists before broad UI enablement.
  - **Validation:** LSP diagnostics clean for modified AI settings, repository, send handler, rate limiting, controller, and tests. `dotnet build Explore.Application/Explore.Application.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false`, `dotnet build Explore.API/Explore.API.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false`, and `dotnet build Explore.Persistence/Explore.Persistence.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false` passed with existing non-AI warnings only. `dotnet build Event.Application.UnitTests/Event.Application.UnitTests.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false` passed with existing warnings. `dotnet test --project Event.Application.UnitTests/Event.Application.UnitTests.csproj --configuration Release --verbosity quiet -p:RunAnalyzers=false -- --treenode-filter "/*/*/*SendAiMessageCommandHandlerTests*/*|/*/*/*AiAssistantSettingGroupTests*/*" --minimum-expected-tests 8 --no-progress --maximum-parallel-tests 1` broadened to the full Application unit suite and passed: 1174 total, 0 failed. API integration controller attribute verification remains blocked by unrelated dirty `KeycloakTokenClient.cs` compile errors, but LSP and API build are clean for the rate-limit attribute.
  - **Effort:** L
