...
- Web Push active-claim and direct notification orchestration fences are implemented and focused-unit green, but remain acceptance-blocked because independent reviewers could not reproduce their PostgreSQL selectors while Docker was unavailable.
...
The narrow OREA-420 durable cache-convergence criterion is independently reviewed and fast-verified. Storage finalization now persists its tenant-scoped provider key before remote I/O, fences before and after provider acknowledgement, exposes upload-session cleanup identity to erasure provider work before local metadata is cleared, and releases only the erased subject's quota reservations. Its fast suites, Release build, and independent race/privacy review pass; the exact PostgreSQL provider-metadata body remains open. OREA-700 documentation convergence is independently accepted. Continue still-open producer/worker/remote-dispatch fence propagation and fresh-scope persisted ownership reload without entering provider-family implementation owned by the concurrent agent. Protected typed materialization uses stable local source IDs and seven-day locator expiry. AI/Listmonk remain blocked on trustworthy remote identifiers; do not infer or hash substitute locators. Do not reintroduce the removed AppHost `FullLocal` condition: every local-data Aspire profile manages the distinct authority resource when `usesExternalPrivacyErasureAuthority` is true.

Docker is currently unavailable. Runtime investigation confirmed both configured sockets are absent, the CLI has no reachable daemon, the test module initializer runs before fixture startup, and serialized selectors fail consistently before database access. This is a host-runtime blocker, not Testcontainers timing or load flakiness. Rerun the provider-metadata, `GlobalLocationPrivacyErasureTests`, finite-expiry, credential-cleanup, Web Push, and notification-intent selectors when a daemon is restored; keep the affected acceptance checkboxes open until then.
