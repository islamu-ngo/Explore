...
- **Task 0.1**: Added primary cross-cutting intent `strong-typing-refactor` to `.agents/contract/intents.yaml`, added matching scenario to `.agents/benchmarks/cold-start-tasks.yaml`, updated `docs/GOVERNANCE.md` Decision Framework, and cleaned up stale missing-triad active paths in archived intents. Routing was historically verified with the former validator, removed by `df999a3`; current changes use `validate-intents` for YAML syntax and architecture tests for semantic routing.
