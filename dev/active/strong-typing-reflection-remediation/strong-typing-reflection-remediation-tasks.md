...
- **Overall status:** Final verification in progress
- **Completed:** 39/39 implementation tasks; Phases 0–5 verified; Phases 6–9 implementation complete
- **Current priority:** Finish the deferred full persistence gate and final cross-phase verification
- **Next recommended slice:** Record terminal gate results, reconcile the workstream/I-VSD status, and stop without committing
...
- **User approval:** Approved on 2026-08-30; expanded on 2026-09-01 to finish the complete workstream
...
- [x] **0.2 Refresh graph and AST evidence and verify every candidate has a semantic category and owning phase.**
  - **Files:** workstream plan Section 2, repository evidence packet, workstream context final-disposition ledger, `tests/Shared/**`, relevant existing mutation projects and `tests/Event.Benchmarks/**` as transitive-consumer evidence, no persistent source-file debt allowlist.
  - **Acceptance:** capture callers, callees, flows, tests, benchmark/fixture consumers, mutation-project consumers, and current AST categories in repository-owned workstream evidence; classify each report item, the nonexistent `AddressGovernancePolicyTests.cs` citation, every transitive helper, and every current `docs/TESTING.md` reflection exception as typed behavior, compiled metadata, machine artifact, physical/protocol metadata, runtime dispatch, source/prose assurance, or cited-but-absent; verify every runtime-dispatch/source-assurance candidate maps to Tasks 2.x–9.x and every retained candidate has an invariant reason; add no new mutation wrapper project while mutation gating remains disabled.
  - **Effort:** M
  - **Dependencies:** 0.1
...
## Phase 2 — Platform Identity Authority [COMPLETED]

- [x] **2.1 Red Phase: add hostile-principal matrices and verify conflicting, malformed, provider-linked, and excluded schemes expose current divergence.**
...
- [x] **2.2 Consolidate Application claim spelling and resolution and verify one fallback implementation remains.**
  ...
  - **Acceptance:** exact raw JWT `sub` and `sid` spellings have one private owner in the canonical Application resolver because the dependency-free Application layer has no exact-value BCL constants; `internal_user_id` has one Application catalog owner; `ClaimTypes.NameIdentifier` remains framework-owned; the GUID fallback remains exact, provider-linked lookup remains separate, and no second platform-ID extraction helper exists in Application; focused canonical identity tests pass.
  - **Effort:** M
  - **Dependencies:** 2.1
  - **Guidance:** Do not move Admin or machine-auth claims into this catalog.

- [x] **2.3 Migrate Infrastructure ambient-user consumers and verify identical resolved IDs and admin outcomes.**
...
- [x] **2.4 Harden Infrastructure AT Protocol adapters and verify the Phase 1 cutover parses once, emits exact scalars, and never leaks through diagnostics.**
...
## Phase 3 — API Authorization And Protocol Literals [COMPLETED]

- [x] **3.1 Characterize Admin, route, and header parity; add the failing DID ingress anchor and verify only the intended malformed-input case is red.**
...
- [x] **3.2 Add the API-local Admin policy and verify no authorization cohort widens or narrows.**
...
- [x] **3.3 Normalize route, relation, and standard header names and verify OpenAPI, HAL, generated client, and response bytes do not drift.**
...
- [x] **3.4 Migrate API DID ingress and verify the public route remains scalar while malformed values fail before MediatR/repository work.**
...
- [x] **3.5 Migrate API ambient-user consumers and verify identity, idempotency, support, and logging remain fail-closed.**
...
## Phase 4 — BFF Claim And Configuration Boundaries [COMPLETED]

- [x] **4.1 Characterize opaque BFF subject/session partitions and configuration precedence and verify the migration has an exact behavior baseline.**
...
- [x] **4.2 Add purpose-specific BFF principal/config helpers and verify token, rate, setup, and session behavior is unchanged except for the approved fail-closed anonymous setup limiter hardening.**
  ...
  - **Acceptance:** helpers expose explicit opaque subject/session/rate-partition purposes, use framework claim/header names, retain configuration precedence, never claim platform GUID authority, and focused BFF tests pass without raw claim logging; authenticated partitions remain purpose-bound and unauthenticated setup attempts use stable remote IP so arbitrary cookie rotation cannot bypass the limiter.
  - **Effort:** L
  - **Dependencies:** 4.1
...
- [x] `dotnet build --configuration Release --verbosity quiet`
- [x] `dotnet test --project tests/Explore.Blazor.IntegrationTests/Explore.Blazor.IntegrationTests.csproj --configuration Release --verbosity quiet` (517 passed; 9 documented clean-HEAD ATProto failures)

## Phase 5 — Typed Application Contract Tests [COMPLETED]

- [x] **5.1 Add direct typed admission anchors and verify they pass alongside the old runtime before deletion.**
...
- [x] **5.2 Convert every admission runtime consumer and reflection-backed port fake and verify invariant-equivalent typed coverage.**
...
- [x] **5.3 Delete the obsolete admission runtime/support dispatch engine and verify zero references remain.**
...
- [x] **5.4 Convert Location positive paths to direct values and verify narrow negative-surface contracts still fail on forbidden API exposure.**
...
- [x] **5.5 Harden Application federation and privacy-erasure DID boundaries and verify the Phase 1 cutover preserves typed live identity plus authority-first tombstoning.**
...
- [x] `dotnet build --configuration Release --verbosity quiet` — 45 projects, 0 errors
- [x] `dotnet test --project tests/Event.Application.UnitTests/Event.Application.UnitTests.csproj --configuration Release --verbosity quiet` — 2001/2001

## Phase 6 — Persistence Behavior And Metadata [VERIFICATION IN PROGRESS]

- **Phase dependencies:** Phases 0, 1, and 5 must have passing phase gates before any Phase 6 task starts.

- [x] **6.1 Red Phase: replace fair-return source tokens with deterministic PostgreSQL ordering/tenant scenarios and verify the new behavioral test detects the missing seam before cleanup.**
...
- [x] **6.2 Convert add-on, recovery, and transfer reflection surfaces and verify money/state-machine/concurrency invariants remain stronger than the removed shape checks.**
...
- [x] **6.3 Convert CLR-backed EF metadata to `typeof`/`nameof` and verify physical/shadow metadata remains explicit.**
...
- [x] **6.4 Delete obsolete persistence reflection/source helpers and verify zero behavior-dispatch/source-scrape references and zero model drift.**
...
- [x] **6.5 Harden persistence DID lookups and verify five-provider product-catalog model parity with unchanged scalar column/index/filter shape and explicit ordinal collations.**
  - **Files:** graph-discovered AT Protocol repositories/configurations, `tests/Event.Persistence.IntegrationTests/Federation/AtprotoFederationPersistenceTests.cs`, `tests/Event.Persistence.IntegrationTests/ConfigurationManifest/ConfigurationManifestAuditProviderMigrationTests.cs`, and generated application-catalog migrations/snapshots under `src/Explore.Persistence*/Migrations/**` for PostgreSQL, SQLite, SQL Server, MariaDB, and MySQL.
  - **Acceptance:** repository inputs accept `AtprotoDid` or its exact `.Value` at the port boundary; queries compare ordinal scalar values; all five product catalogs retain the existing DID column name/type/length, unique index, and filter shape; provider models record PostgreSQL `C`, SQLite `BINARY`, SQL Server `Latin1_General_100_BIN2`, and MariaDB/MySQL `ascii_bin`; migrations/snapshots are generated through `dotnet ef` with no hand edits or compatibility shim; all five contexts report no pending model changes after generation; DataProtection/privacy-authority catalogs remain excluded because they do not persist AT Protocol identity state.
  - **Effort:** L
  - **Dependencies:** Phase 1
  - **Guidance:** Scenario 3.8; the 2026-09-01 re-baseline permits only the explicit portable ordinal collation delta. Any other model drift blocks the phase.
  - **Progress:** typed repository/query implementation and all five generated migrations/snapshots are complete; final pending-model and provider behavior gates are deferred to the end by user instruction.
...
## Phase 7 — Generated Contract Determinism [COMPLETED]

- **Phase dependencies:** Phases 1 and 3 must have passing phase gates before any Phase 7 task starts.

- [x] **7.1 Add generated-contract regression coverage and verify DID, route, operation, and DTO wire shapes remain unchanged.**
  ...
  - **Progress:** parser-based real-artifact and synthetic-mutation regression code is complete; execution is deferred to the final verification pass.

- [x] **7.2 Run the canonical API-first generation chain and verify schema, client, and inventory remain byte-identical to the phase baseline.**
  ...
  - **Progress:** all three generators completed and the schema/client/inventory hashes are byte-identical to baseline; final build/test gates are deferred.

### Phase 7 Verification — Run Once After Tasks 7.1–7.2

- [x] `dotnet msbuild src/Explore.API/Explore.API.csproj -target:GenerateOpenApiDocuments -property:Configuration=Release`
- [x] `dotnet msbuild src/Explore.Blazor.Client/Explore.Blazor.Client.csproj -target:GenerateApiClient -property:Configuration=Release`
- [x] `dotnet run --project eng/tools/Explore.ApiContractInventory/Explore.ApiContractInventory.csproj --configuration Release --no-launch-profile`
- [x] `dotnet build --configuration Release --verbosity quiet`
- [x] `dotnet test --project tests/Explore.GeneratedContracts.Tests/Explore.GeneratedContracts.Tests.csproj --configuration Release --verbosity quiet`

## Phase 8 — Typed Blazor Contract Tests [COMPLETED]

- **Phase dependencies:** Phases 4 and 7 must have passing phase gates before any Phase 8 task starts.

- [x] **8.1 Convert tenant directory-operator component tests and verify typed parameters preserve HAL, read-only, focus, live-region, conflict, and validation behavior.**
...
- [x] **8.2 Convert tenant directory-operator service tests and verify exact HAL edit mapping, grouped patch, revision, and conflict behavior through the public service.**
...
- [x] **8.3 Convert participant-readiness and waitlist tests and remove redundant transfer existence reflection while verifying HAL-driven rendered behavior.**
...
- [x] **8.4 Delete the shallow client auth-state claim wrapper and verify dock persistence uses server/framework-confirmed authentication only.**
...
- [x] **8.5 Remove local organization-member authority inference and verify only exact HAL relations render edit/delete actions.**
...
- [x] **8.6 Classify remaining test-only `DynamicComponent`/runtime type uses and verify compile-time dependencies are migrated without changing legitimate runtime composition.**
...
## Phase 9 — Recurrence Guard And Release Closure [COMPLETED]

- **Phase dependencies:** Phases 0–8 must have passing phase gates before any Phase 9 task starts.

- [x] **9.1 Red Phase: add synthetic Roslyn audit fixtures and verify prohibited runtime dispatch/source assurance fails while legitimate metadata/artifact parsing passes.**
...
- [x] **9.2 Implement the repository-owned assurance audit and verify deterministic category/location diagnostics.**
...
- [x] **9.3 Migrate remaining prohibited assurance and enable the guard and verify zero findings, zero generated/model drift, and plan-aligned I-VSD/docs.**
...
- [x] **9.4 Changelog contribution and final commit composition: create/validate the change fragment and verify release impacts and terminal footer are complete; commit only with explicit user authorization.**
...
- **Public API or additional schema change:** not planned. Task 6.5 permits only generated provider-native DID collation metadata; any further discovered need requires plan, I-VSD, OpenAPI, release, and task re-baselining before implementation.
