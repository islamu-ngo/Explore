...
- [ ] Write ADR: PostgreSQL-owned `EmailDispatchOutbox` state machine.
...
- [ ] Decide whether MQContract remains the RabbitMQ abstraction.
...
- [ ] Write async workflow failure matrix.
...
- [ ] Add first E2E path.
  ...
  - Validation: documented project-specific test command; no solution-level `dotnet test`.
...
- [ ] Add admin/status endpoints after state machine is stable.
  ...
  - Validation: API integration and architecture tests.
- [ ] Add replay/park/pause/resume write actions after transition rules are stable.
  - Acceptance: action availability comes only from HAL links; UI must not infer from roles/claims/local state.
  - Validation: HAL affordance tests and authorization tests.
...
- [ ] Log durable findings if non-obvious implementation behavior is discovered.
