...
- [x] **4.3 Add operator-facing projection/quota signals**
  ...
  - **Validation:** focused projection status query tests passed 2/2; focused event projection rebuild tests passed 7/7 after adding projection quota metric emission; scoped docs updated with bounded metric tags and triage states. Full Release build remains blocked by unrelated generated Blazor client anonymous HAL link type mismatches.
  - **Effort:** M
  - **Dependencies:** 4.1
...
- [ ] `dotnet test --project Event.Architecture.Tests/Event.Architecture.Tests.csproj --configuration Release --verbosity quiet` passes. Previously verified 2026-05-28 with 178 succeeded and 1 known skipped response-metadata test; latest semantic-reservation-slice attempt fails on unrelated `EmailDispatchStatusLinkPolicy` permission metadata.
