...
| Deployment-detected Keycloak must remain sanitized and manageable after configuration. | Implemented: `AuthProviderConfigurationService`, API compatibility mapping, Compose/Aspire propagation, server-derived secret ownership, and focused service/TestServer tests | High | Complete authority/client-ID tuples report detected/configured state without returning the secret; HAL navigation still reaches the provider page for create, repair, and reconcile actions. Real deployed login/realm-repair E2E remains open. |
