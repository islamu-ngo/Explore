...
- [x] **9.3 Integrate setup access, authentication provider, and OIDC handoff**
  ...
  - **Evidence:** The original setup-entry characterization passed 1/1 before production edits. The corrected source contract then failed 0/1 against the rejected workspace wrapper at the missing split-panel assertion. After restoration, `SetupSourceTests` pass 3/3 and `SetupTests` pass 13/13, including both authoritative Keycloak post-entry destinations and explicit authentication configuration routing. The authentication workspace was then re-baselined against the approved right-rail reference: the setup summary occupies 25% at desktop and 33% at tablet, existing theme/language controls dock at its top, main progress names only the current step, and mobile uses a sticky native menu disclosure. Shared Release build and focused tests pass; fresh captures live under `.omo/evidence/onboarding-unified-workspace/task-3/`, and the scrolled mobile check reports `beforeTop=0`, `afterTop=0`, `menuOpen=true`, `overflow=false`, and `switchersVisible=true`.
  - **Effort:** L
  - **Dependencies:** 9.2
