...
> **Status:** Draft — Phase 0 approval required  
...
## Phase 0 — Architecture and Policy Baseline

- [ ] **0.1 Approve logical intent, channel delivery, and transport work.**
...
- [ ] **0.2 Approve reporter consent, withdrawal, and single-SLA policy.**
...
- [ ] **0.3 Approve immutable occurrence, audience, coalescing, and supersession policy.**
...
- [ ] **0.4 Approve retention, redaction, fairness, observability, and operator policy.**
...
- [ ] **0.5 Persist a unique Coop callback effect pointer.**
  - Files: incoming-webhook contracts/handler, effect/outbox entities and configurations, migration if required, serialization, focused persistence/API tests.
  - Acceptance: `coop` + `moderation.coop.decision` creates one pointer effect per inbox/effect kind using the unique provider event/delivery identity; no raw callback payload enters the pointer or logs; FK/retention rules prevent an orphaned pointer.
...
- [ ] **1.1 Add explicit intent/delivery/email schema relationships and backfill.**
  - Files: `NotificationIntent`, `NotificationDelivery`, `EmailDispatchOutbox`, lookup enums/entities, EF configurations, new reversible migration, model snapshot, `LookupTableSeeder`, `schemas/islamu-event.md`, domain/persistence tests.
  - Acceptance: delivery has channel/policy, required/optional snapshot, channel-neutral outcomes, and optional notification/email links; email has non-null intent FK and renamed/backfilled `RecipientUserId`; composite keys prevent tenant/intent/recipient mismatch and cross-intent delivery links; unique constraints match the plan; legacy rows receive deterministic intents/deliveries before constraints become required; migration-local lookup/status inserts and seeder repair have parity.
...
- [ ] **1.3 Implement exact unique-conflict recovery and batch insert semantics.**
  ...
  - Acceptance: two workers for one occurrence/user produce one intent and one row per channel; exact conflict rolls back before fresh-transaction load/repair; unrelated DB errors throw; rollback after intent or email insertion commits nothing; fanout batch path uses `ON CONFLICT DO NOTHING` where proven to avoid N+1 checks.
...
Phase 1 gate: Release build plus full `Event.Domain.UnitTests`, `Event.Application.UnitTests`, `Event.Persistence.IntegrationTests`, `Explore.Infrastructure.Tests`, and `Event.Architecture.Tests`.
...
- [ ] **3.2 Implement deterministic event/session audience queries.**
  ...
  - Acceptance: cutoff/current-status predicates are exact; event-wide and target-session cohorts follow the plan; whole-event/day/session scope, partially cancelled children, pending/waitlisted/approved parents, registrations after cutoff, and duplicate children are covered; cursor is `(FirstEligibleRegistrationCreatedAt, UserId)`.
...
- [ ] **5.1 Split reporter consent in the domain and database.**
  ...
  - Acceptance: `ReportCaseUpdatesConsent` and `ReportFollowUpContactConsent` replace the old field; legacy true maps only to follow-up consent because that is what the old wording authorized; case-update consent backfills false; aggregate method changes consent with audit/concurrency semantics.
...
- [ ] **5.6 Queue report receipt using the canonical SLA.**
  ...
  - Acceptance: report/case/provider-sync/in-app/email state commit atomically; case-update consent gates email; current verified address is required; copy thanks the reporter, derives the review window only from `CaseSlaHours`, and excludes evidence/fingerprints; no consent or email records a typed skipped channel, not a second fallback intent.
...
- [ ] **5.7 Queue safe final outcome after successful enforcement.**
  ...
  - Acceptance: NoViolation/Duplicate receive safe no-action copy; LightModerate/HeavyRedact/WarnOrganizer receive safe action-taken copy; completion and notification channels commit together after enforcement; failed, escalated, already-closed, and replay paths create no false/duplicate final email.
...
- [ ] **7.1 Schedule one reminder for an approved registration.**
  ...
  - Acceptance: only Approved parent and target child qualify; earliest covered published session determines one reminder; lead time is validated/documented; multi-session rows do not duplicate; optional in-app/email preference behavior is explicit.
...
- [ ] **7.3 Prove timezone and DST reprojection.**
  ...
  - Acceptance: UTC scheduling remains correct across DST gap/overlap, event timezone change, and local display reprojection; old pointer remains harmless; one live replacement reminder exists; test cases record exact zones/instants.

Phase 7 gate: Release build plus full `Event.Application.UnitTests`, `Event.Persistence.IntegrationTests`, `Explore.Infrastructure.Tests`, and `Event.Architecture.Tests`.
...
| 1 | `Event.Domain.UnitTests`, `Event.Application.UnitTests`, `Event.Persistence.IntegrationTests`, `Explore.Infrastructure.Tests`, `Event.Architecture.Tests` |
...
| 7 | `Event.Application.UnitTests`, `Event.Persistence.IntegrationTests`, `Explore.Infrastructure.Tests`, `Event.Architecture.Tests` |
