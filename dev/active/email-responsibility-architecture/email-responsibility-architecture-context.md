...
> **Progress:** 31/51 implementation tasks complete; phase verification remains separate
> **Current phase:** Phase 3 fanout execution; Phase 1 runtime verification remains open
> **Next task:** 3.6b add fanout processor settings, backpressure, telemetry, and health
...
- Task 3.6b: processor hosting/settings/backpressure plus fanout telemetry and health.
...
Start Task 3.6b from the independently confirmed fair claim contract and page processor. Add bounded processor settings/validation, a hosted processor that creates a fresh DI scope per claim, global/per-tenant scheduling bounds that pass their authoritative ceiling into `ClaimDueRoundAsync`, and high/low-watermark optional-reminder backpressure that never blocks required work. Add PII-free backlog/oldest-age/processed/remaining/lease-contention/supersession metrics and readiness. Reuse existing `BusinessMetrics`, health conventions, and configured service-registration patterns; do not wire event/report/moderation triggers, render new templates, or activate reminders yet. Keep Task 5.9 disabled until the fresh full API evidence is recorded. The working tree currently contains unrelated ATProto/auth/location-privacy work; do not modify, stage, revert, or include it. Use only the main checkout and never create a linked worktree or `.worktrees` directory. At every handoff, synchronize progress, evidence, risks, and the next task across context, plan, and tasks.

## Handoff Notes

### Handoff — 2026-07-19 Europe/Brussels

- **Current state:** 31/51 implementation tasks are complete. Task 1.5 and the Task 1.6a production boundary are committed in `9bfaf1e0`; later SMTP operations, their migrations, Tasks 3.4a–3.6a, the strict controller guard, and workstream docs are preserved main-checkout changes. Task 3.6b is next.
- **Next action:** implement the scoped fanout processor, bounded settings/backpressure, and PII-free metrics/health on the authoritative fair-claim contract.
...
- **Modified files:** Task 3.6a adds the claim-round contract, fair/ceiling-aware run repository operations, one partial global occurrence index/config/migration/snapshot entry, and focused PostgreSQL source. The full snapshot was repaired from the latest location/ATProto Designer rather than reverted to HEAD. Extensive unrelated ATProto/auth/location-privacy and SMTP work remains interleaved and must remain untouched.
- **Validation:** Task 3.6a is independently confirmed after the event-precedence race and empty-snapshot blockers were fixed. Application and Persistence product builds passed; scoped diff checks are green; `.worktrees` is absent. Tests and test-project builds were not run per user instruction, so live PostgreSQL fairness/concurrency/index evidence and the Phase 3 gate remain open.
- **Documentation impact:** plan/context/tasks record 31/51 complete and advance the current task to 3.6b without overstating test or PostgreSQL evidence.
- **Risks:** Task 3.6b must prevent processor-level starvation/backlog feedback loops, keep every claim in a fresh scope, and expose useful health/metrics without PII or high-cardinality tenant labels.
