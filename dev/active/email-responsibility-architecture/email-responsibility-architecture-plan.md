...
> **Implementation progress:** 27/51 tasks complete; phase verification remains tracked separately
...
Tasks 3.1–3.4b are implemented: immutable occurrence persistence/pointers, deterministic audience paging with `CoverageEstablishedAt`, fenced lease/checkpoint recovery, idempotent general-outbox-to-pending-run handoff, and typed occurrence-recipient materialization are complete. The remaining work is split into five bounded slices:
...
- **3.5b — Suppress already-materialized work at the handoff fence** (`M`, depends on 3.5a): mark linked channel deliveries superseded, skip pending/retryable pre-handoff email with a stable reason, preserve sent/unknown/post-handoff evidence, and recheck occurrence state in dispatch eligibility.
- **3.6a — Add fair runnable selection and cross-instance tenant claim limits** (`L`, depends on 3.5b): select one due occurrence per tenant per round, order by required priority then time/ID, use tenant advisory locking for the configured active-claim ceiling, and add only the index proven necessary for the global scan.
...
The highest-risk near-term area is now crash-safe fanout page execution. Task 3.4c must commit every recipient outcome before advancing the compound cursor, stop on a stale lease/fence, and let replay converge through the independently confirmed occurrence/user uniqueness path without gaps or duplicates. Task 3.4b now provides the closed v1 snapshot-only materializer and recipient-bound location mask. The compiled PostgreSQL email/fanout scenarios and explicit Mailpit lane remain runtime-unproven because tests were stopped and the separately recorded shared-fixture migration failure has not been revalidated.
