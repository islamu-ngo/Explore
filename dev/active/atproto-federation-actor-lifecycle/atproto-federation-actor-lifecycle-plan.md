...
- **Implemented:** Actor and exact identity suspend/reinstate transitions are instance-admin-only, idempotent on same-state retries, and append immutable records only on real transitions. Commands reuse `InstanceSetting` update authorization for `global-actor-moderation` and handlers recheck instance-admin status.
- **Review remediations:** Grounded `PdsSyncOutbox` work transactionally converts settled ownership and pending/processing Event mutations to fenced Delete compensation when Actor or exact-DID suspension makes publication ineligible; exact-identity reconciliation is DID-scoped, and soft-deleted source Events remain selectable for cleanup. One central eligibility policy now gates public actions, locations, program, agenda, days, sessions, session languages, aspects, sitemap, AI reference search, and Open Graph reads. Local echoes require exact outbound ownership plus current local eligibility. Global moderation evicts tagged Event-detail HybridCache entries and five process-local output-cache tags, `event-discovery`, `public-home-discovery`, `list-data`, `detail-data`, and `seo-sitemap`; this does not provide cross-replica consistency. The four moderation operations publish `WritePolicy` and typed `429` metadata, and OpenAPI plus the generated client were regenerated from source.
- **Eligibility boundary:** Public eligibility, anonymous child reads, inbound projection lifecycle, management visibility/HAL, cache invalidation, and outbound publication compensation compose current Actor, identity, participation, Event, presentation, ownership, tombstone, session, source-version, record-key, and CID state. Creation eligibility remains separately stricter for Organization and Group organizer eligibility. RSVP behavior is unchanged.
- **Contract impact:** Four reason-only POST routes use server-selected actions. Additive authenticated management reads for Event days, session languages, Islamic aspects, and Tech aspects keep management clients off the fail-closed public routes. MCP and Blazor select those generated operations for management flows. Management Event collection HAL uses management detail/session relations and omits public report affordances. No schema, migration, or Cerbos policy change belongs to this slice.
- **Verification status:** The canonical Release build passed with 0 errors; pre-existing package advisory warnings remain. Focused evidence passed for moderation compensation 17/17, public actions 10/10, local echo 3/3, Actor moderation API 9/9, cache invalidation 1/1, public eligibility 3/3, managed child Application/API/Blazor lanes, management collection HAL 23/23, and publication planning 49/49. The full Persistence run confirmed all three in-memory eligibility tests before 597 Docker/Testcontainers failures. Final architecture verification remained 320/330; its same nine failures belong to existing privacy/cache registries, naming, generated-client, authorization-parity, update-inventory, and concurrent persistence/ticketing work. Final direct review passed after one-request-per-handler remediation, with a green Release build, Application 4/4, API 2/2, diagnostics, conflict scan, and diff check.
