...
Gate to exit Milestone A:

- shared-definition CRUD stable
- governance policy stable
- duplicate machine-key rules stable
...
- rebuildable event projection baseline

Explicitly out of scope for this milestone:

- full operator-driven sync UX
- complex merge flows
- advanced moderation/export consumers

Gate to exit Milestone B:

- event instantiation stable
- event-local runtime reads/writes stable
- event projection baseline stable
- provenance tests green
...
- this milestone mirrors a proven Event pattern; it does not co-design Event and EventSession under active churn

Gate to exit Milestone C:

- session template/runtime baseline stable
- session-local reads/writes stable
- session projection baseline stable
- session provenance tests green
...
- export/publication projection integration
- rebuild tooling
- projection consistency rules

Baseline projection goal:

- enough projection behavior for critical query paths

Advanced projection goal:

- moderation/analytics-specific consumers after baseline proves itself
...
- diff logic
- operator-driven merge/sync decisions
- stale-version conflict handling
- admin API / UX for sync

Rule:

- full sync is a later milestone, not a prerequisite for baseline runtime shipping

#### Milestone F - Aggregate Read Views And Publication Contracts

Scope:

- event-with-sessions aggregate DTOs/views
- publication/export composition
- lexicon-facing planning and canonical/view contract alignment

Rule:

- lexicon strategy informs architecture now, but is not a dependency for Milestones A-C shipping
...
Sync is explicit and version-aware, but full diff/sync delivery is a later milestone.

Baseline implementation must include:

- instantiation
- provenance/version stamping
- source-id-first lineage

Full operator-driven diff/sync workflows are intentionally sequenced after baseline runtime flows stabilize.

When implemented, sync works as follows:
...
### Projection Consistency Baseline

This plan must choose and document a baseline consistency model before broad projection integration expands.

Recommended baseline:

1. runtime/source-of-truth rows are written transactionally
2. projection rows are updated in the same transaction for the first critical query paths
3. rebuild tooling exists for recovery and reindexing
4. advanced eventual-consistency/outbox flows can come later if scaling requires them

This keeps the first implementation operationally simple and debuggable.
...
- moderation-specific projection consumers
- analytics-specific projection consumers
- eventual-consistency/outbox mechanics if justified by scale

### Concurrency And Versioning Rules

The implementation must explicitly handle:

- concurrent edits to runtime definitions
- concurrent edits to templates
- option replacement races
- projection rebuild replay safety
- stale-version sync attempts

Expected direction:

1. optimistic concurrency is preferred for template/runtime aggregates
2. projection rebuilds must be idempotent and replay-safe
3. sync must detect stale provenance/version mismatches before applying changes
4. version stamping is required before exposing full operator-driven sync workflows
