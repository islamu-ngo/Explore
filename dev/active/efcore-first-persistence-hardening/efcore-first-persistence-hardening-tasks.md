...
- **Overall:** BLOCKED ON FINAL MONOLITHIC TEST BASELINE
- **Implementation approval:** RECORDED
- **Current phase:** Phase 8 — Task 8.8 verification blocker
- **Completed implementation tasks:** 107
- **Plan:**
...
## Phase 0: Baseline And Invariant Breakers — COMPLETE

- [x] **0.1 Obtain explicit implementation approval.**
  Verification: context records the approval revision; no product edit precedes it.

- [x] **0.2 Refresh persistence impact graph.**
...
- [x] **0.3 Establish code baseline and classify inherited failures.**
  Verification: Release build exits zero; deterministic persistence and
  architecture baselines are captured; every inherited failure category is
  mapped to an exact owning phase and remains mandatory at that phase and the
  final gate.

- [x] **0.4 Capture machine-readable violation baseline.**
...
- [x] **0.5 RED provider physical-name mismatch tests.**
...
- [x] **0.6 RED tenant isolation mutation tests.**
  Scenario: `PERSIST-S3`.  
...
- [x] **0.7 RED critical concurrency invariant tests.**
...
- [x] **0.8 RED zero-sensitive evidence tests.**
...
- [x] **0.9 Record performance and query baseline.**
...
## Phase 2: Naming And Model Normalization — IN PROGRESS

- [x] **2.1 RED finalized naming matrix tests.**
...
- [x] **2.2 GREEN normalize tenancy and identity mappings.**
...
- [x] **2.3 GREEN normalize organization and group mappings.**
...
- [x] **2.4 GREEN normalize event and location mappings.**
...
- [x] **2.5 GREEN normalize registration and admission mappings.**
...
- [x] **2.6 GREEN normalize payment and refund mappings.**
...
- [x] **2.7 GREEN normalize notification and email mappings.**
...
- [x] **2.8 GREEN normalize webhook and federation mappings.**
...
- [x] **2.9 GREEN normalize settings and theme mappings.**
...
- [x] **2.13 Complete Phase 2 verification gate.**
...
- [x] **3.1 RED schedule move provider tests.**
...
- [x] **3.2 GREEN convert schedule move mutations.**
...
- [x] **3.3 RED inventory transition provider tests.**
...
- [x] **3.4 GREEN convert inventory transitions.**
...
- [x] **3.9 RED webhook persistence parity tests.**
...
- [x] **3.10 GREEN convert ordinary webhook mutations.**
...
- [x] **3.17 Remove completed repository exception entries.**
  Verification: registry contains no converted repository/type and architecture
  tests remain green.

- [x] **3.18 Complete Phase 3 verification gate.**
...
- [x] **4.5 RED queue claim starvation tests.**
...
- [x] **4.6 GREEN select minimal queue strategy.**
...
- [x] **4.7 RED relational named-lock lifecycle tests.**
  Scenario: `PERSIST-S4`.  
...
- [x] **4.10 GREEN remove avoidable clock SQL.**
...
- [x] **4.11 Complete Phase 4 verification gate.**
...
- [x] **5.1 RED final provider-boundary architecture test.**
...
- [x] **5.2 GREEN centralize provider detection.**
...
- [x] **5.3 GREEN isolate advisory/application locks.**
...
- [x] **5.4 GREEN isolate pessimistic row locks.**
...
- [x] **6.1 RED migration service compatibility tests.**
...
- [x] **6.2 GREEN isolate provider generator internals.**
...
- [x] **6.3 RED obsolete model-differ behavior tests.**
  Scenario: `PERSIST-S7`.  
...
- [x] **6.12 Inspect generated Up and Down operations.**
...
- [x] **8.2 Update testing and operations documentation.**
...
- [x] **8.3 Update configuration and recovery documentation.**
...
- [x] **8.4 Update schema and release artifacts.**
...
- [x] **8.7 Revalidate I-VSD final mappings.**
...
- [ ] **8.8 Run final required build and tests.**  
  ...
  exit zero.
  Blocker: the Release build exits zero and all persistence-owned focused
  architecture, provider, migration, concurrency, projection, API seam, and
  111 mutation-contract tests are green. The monolithic persistence project
  still exposes stale pre-hardening fixtures that contradict the development
  rebaseline, including session fixtures that set `LocationId` without the
  required `EventLocationId`, incremental-migration backfill assertions against
  the single generated initial catalog, and test-process EF internal-provider
  cache exhaustion. The full architecture project also retains its exact nine
  unrelated baseline failures. These tests were not skipped, suppressed, or
  weakened.

- [x] **8.9 Run markdown links and diff hygiene.**
  Verification: documentation links resolve and `git diff --check` exits zero.

- [x] **8.10 Reconcile final task and context state.**
...
Task 8.8 is blocked on reconciling the repository-wide persistence integration
baseline with the approved development-only generated initial catalogs and
current location-mediation invariant. No product behavior, provider,
compatibility shim, or migration artifact is deferred. A new provider, provider
extension dependency, removal of configurable schemas, or replacement of
tracked migration history remains a separate Project-Steward decision.
