...
Last Updated: 2026-07-30 Europe/Brussels

## Status Summary
- **Overall status:** Phase 5 Wave 1 is independently source-confirmed/runtime-blocked. Task 18's persistence executor errored after partial entities/configurations/worker work; recovery Task 28 now owns full 5.2/5.3 completion. Task 19 gates every API/security unlock. Phase 4 ticketing and Phase 5 idempotency PostgreSQL races remain Docker-blocked, and database application/runtime rollout remains unevidenced
- **Checkbox count:** 23/88 implementation-task boxes are checked. This is not verified Phase 5 or workstream completion
- **Current priority:** Complete recovery Task 28 and close Task 18 only after every persistence/hold acceptance has an exact receipt
- **Next recommended slice:** Independently verify the combined Task 18/28 result under Task 19; do not unlock API/security or report unavailable PostgreSQL races as passes
- **Latest focused evidence:** Phase43 ticketing Application 59/59; schedule-target deletion guards 3/3; ticketing persistence 13/13; EventSeries repository pricing regression 1/1; Phase43 ticketing API 11/11; platform-monetization API 10/10; EventDay and EventSession deletion-conflict API checks 1/1 each; ticketing inventory architecture 1/1; semantic registry 2/2; full Release build 0 errors and 2,628 warnings. OpenAPI and NSwag artifacts were regenerated. Both PostgreSQL row-lock concurrency tests are blocked before execution only because Docker is unavailable at `unix:///var/run/docker.sock`
...
## Phase 5: Registration Orders, Inventory Holds, Guest Checkout Core 🟡 WAVE 2 PERSISTENCE/HOLDS IN PROGRESS
...
- **Task 18 recovery:** its original executor errored after partial entity/configuration/worker work. Task 28 must complete and evidence all remaining 5.2/5.3 requirements before the coordinator closes Task 18; Task 19 and Task 20 remain blocked.
...
- [ ] **5.2 Order lines with snapshots + buyer-chosen prices** — `RegistrationOrderLine.cs` + config (new) incl. `ChosenUnitPriceAmountSnapshot`, `TicketPricingModeSnapshot`, `PlatformFeePolicyVersionSnapshot` — **Acceptance:** catalog revision leaves lines byte-identical; chosen price validated against **pinned** version bounds (below-minimum rejected; 0 accepted when minimum 0) — **Effort:** M — **Dependencies:** 5.1
  - **Progress:** team Task 2 implemented immutable order-line snapshot/domain rules with 2/2 executor-reported focused tests. Persistence configuration and independent confirmation remain pending.
...
- [ ] **5.10 Platform-contribution order component + organizer-earnings transparency** — `RegistrationOrderPlatformContribution.cs` + config (new); `IOrganizerEarningsCalculator` + pure decimal implementation (new); order-totals composition; checkout DTO additions — **Acceptance:** hidden when instance-disabled; 0 selection stores no row; amounts computed server-side only; organizer earnings = line totals − fee policy exactly (decimal/rounding tests); contribution never leaks into organizer earnings/totals/exports; positive total → `AwaitingPayment`, all-zero → free path — **Effort:** L — **Dependencies:** 4.5, 5.2
  - **Progress:** team Task 2 implemented the optional contribution domain component, registered the earnings calculator, and reports 2/2 focused calculator tests. Persistence configuration, checkout/API composition, exact receipt, and independent confirmation remain pending.
