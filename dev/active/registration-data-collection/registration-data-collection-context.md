...
## SESSION PROGRESS (2026-07-20 Europe/Brussels)
...
| `dev/active/registration-data-collection/hi-events-report.md` | Existing | Docs | Hi.Events research — behavior catalog, §7 defect-derived acceptance criteria, §9 adopt/adapt/reject, §10 AGPLv3 reuse rule, §11.4 deferred inventory | Do not edit; cited from Phases 4–8 + Task 14.8; never an architecture authority |
...
- **D19** Hi.Events = behavior catalog under AGPLv3: adopt UX/workflow lessons, adapt concepts, reject its persistence/authorization/money/idempotency machinery; branding demand not binding; provenance pinning (`9de8863a`) for any copied code; deferred breadth lives only in Task 14.8.
...
- Hi.Events reject-list is binding (plan §4.13): no mutable published prices, no JSON canonical answers, no public/display IDs as authorization, no cache-only idempotency, no float money, no attendee-derived inventory release, no external calls inside transactions; never add "Powered by Hi.Events" branding; record commit+path provenance for any copied code.
...
### Handoff — 2026-07-20 Europe/Brussels (updated after Hi.Events re-baseline, same day)
- **Current state:** Planning artifacts complete and re-baselined with `hi-events-report.md` findings + D17/D18/D19 (pricing modes, instance monetization, Hi.Events boundary); 88 tasks across P0–P14; no runtime code changed; workstream Draft awaiting user review.
