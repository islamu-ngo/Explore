...
**Erasure-authority topology:** [`dev/active/optional-retained-erasure-authority/optional-retained-erasure-authority-plan.md`](../optional-retained-erasure-authority/optional-retained-erasure-authority-plan.md) owns this topology. Missing `LocationPrivacy:ErasureDurability:Mode` means the one-database `ApplicationDatabase` default; only explicit `RetainedAuthority` enables the independently retained authority and synchronous pre-host replay, with no fallback. Only `local-full` auto-provisions it, and Persistence/EF Core owns both ledger schemas, repositories, contexts, and generated migrations.
...
- Corrected task ownership and execution order: ELP-420 is split into additive/final generation; historical ELP-505/515 acceptance is transferred to the focused OREA transaction/persistence tasks with no executable ELP checkbox; ELP-230C runs after every consumer migration; and infrastructure plus manual browser verification are mandatory.
...
1. Execute remaining ELP W9 work: `ELP-350` before `ELP-315`, plus `ELP-360` and `ELP-510`; OREA independently owns the transferred two-mode erasure/outbox workflow and its local-ledger persistence prerequisites.
...
- ELP-405 implementation evidence: the event-scoped `EventLocationController` exposes anonymous public disclosure at the canonical controller root, authenticated registration-aware disclosure at `/my-access`, authenticated/audited management disclosure at `/{eventLocationId}/management`, and the resource-authorized concurrency-checked disclosure PUT. Public and private requests dispatch distinct CQRS requests into the same ELP-315 disclosure authority; requester identity is server-derived, management authorization/audit remains fail closed, and hidden results never expose a physical entity. Public has no shared output-cache annotation, private routes use `[PrivateNoStore]`, and the management HAL update candidate carries the same event-update resource metadata as the command. Route names, assembler/policy registration, HAL schema registration, and the API changelog are present. Application Release compiled 2 projects with 0 errors and 0 warnings; API Release compilation with build-time OpenAPI generation disabled compiled 8 projects with 0 errors and 90 unrelated warnings; focused physical-field/auth/cache scanning and `git diff --check` passed. Tests and ELP-420A generated artifacts remain deferred by instruction, so ELP-405 is not checked. Next code task: ELP-520.
- ELP-520 implementation evidence: all three location-privacy correction producers have exact normal-delivery and dead-letter-reconciliation cases in `CompositeOutboxMessageDispatcher`, and unknown general-outbox types fail closed with an exception. `LocationPrivacyCorrectionDispatcher` rejects malformed or unmapped JSON, validates aggregate identity plus UUIDv7/UTC correction envelopes, invalidates bounded HybridCache tags, and sends projection corrections only through the network-free ATProto planner. The planner uses the correction outbox message ID as its durable idempotency key, so replay either creates one PDS sync row or returns the bounded `correction_already_planned`; absent remote records are an explicit settled state. `OutboxProcessor` and `OutboxRepository` provide optimistic processing/reconciliation leases, exponential retries, retained dead-letter rows, and recurring reconciliation, while control-plane operations expose failed/dead-lettered counts. No missing production route or recovery branch was found, so no duplicate dispatcher abstraction was added. Infrastructure Release compiled 4 projects and API Release compiled 8 projects with 0 errors and 0 warnings; focused event-route, payload-field, registration, visibility, and scoped diff checks passed. Tests and Docker remain skipped by instruction, so ELP-520 is not checked. Next code task: ELP-410.
- ELP-410 implementation evidence: the API already has separate EventLocation management detail/collection policies, a registered `EventLocationResourceAssembler`, and one concrete `edit` candidate for the disclosure PUT. The candidate reuses the direct command's event-update authorization kind/action, validates route/event/tenant/actor/scope parity before evaluation, and is omitted on malformed metadata or authorization failure. The existing focused contract explicitly rejects speculative owner/remediation relations; those stay absent until ELP-620/530 add executable routes. API Release compiled 8 projects with 0 errors and 0 warnings. Tests remain skipped, and the downstream routes remain open, so ELP-410 is not checked.
- ELP-440 implementation evidence: public and attendee calendar exports use separate request/handler/DTO contracts and the same Ical.Net serializer. Public export loads only published/public content and resolves one `Public` disclosure; attendee export is authenticated, `[PrivateNoStore]`, and resolves one registration-scoped `Attendee` disclosure with requester identity supplied by the disclosure service. Neither DTO exposes physical IDs or location entity fields, and the canonical subscription URL contains only the event URL. Both routes return the explicit third-party retention warning header. Application Release compiled 2 projects and API Release compiled 8 projects with 0 errors and 0 warnings; focused purpose/auth/cache/contract scans passed. Tests and Docker remain skipped, so ELP-440 is not checked. Next code task: ELP-720.
- ELP-720 implementation evidence: `EventManagementMcpTools` no longer applies the Stage-A zero-location ceiling to public session/program outputs. It accepts only ELP-320 `EventLocationPublicDto` values, flattens and deduplicates them into one `IAiContextGateway.SanitizeMany` batch, and refuses serialization unless every envelope succeeds with exact entity, cardinality, field name, `Allow` rule, and value parity and with no denial or redaction. `EventMcpLocationDescriptor` is nested in public session, program-group, and program-item descriptors, bounded with the existing MCP string limits, and never maps legacy `LocationId`/location/room fields. `AiContextDisclosureRegistry` keeps raw `LocationPii` entries Restricted/Phase-4-gated and separately registers already-evaluated public projection fields, preserving PII reflection count/inventory semantics. Federation/PDS inspection found no missing production migration: `AtprotoEventPublicationSnapshotFactory` batches only event-scoped public disclosure inputs and maps only evaluator results; `LocationPrivacyCorrectionDispatcher` validates PII-free payloads, invalidates scoped caches, and plans PDS correction with the UUIDv7 outbox message ID as its idempotency key, accepting only already-planned or absent-remote settled states. Application Release compiled 2 projects with 0 errors and 294 existing warnings; API Release compiled 8 projects with 0 errors and 927 existing warnings; `git diff --check` passed. Tests and Docker remain skipped, so ELP-720 is not checked. Next code task: ELP-730.
- ELP-730 production absence evidence: a complete `src/` scan found no `LocationDiscoveryPoint`, spatial table/index, `ST_DWithin`, `ST_Distance`, geography point mapping, NetTopologySuite, or PostGIS dependency. `PublicDiscoveryAreasConfig` stores explicitly governed coarse area metadata and limits optional centroids to two decimal places; configured Location IDs are checked with `GetExistingTenantLocationIdsAsync`, whose Persistence query is tenant-bounded, `IgnoreAutoIncludes`, and ID-only. Those internal IDs only filter existing public event queries and never enter `PublicDiscoveryAreaDto`. `GetHomeDiscoveryQueryHandler.MapDiscoveryItem` copies event/federation values only, leaving reserved distance, nearest session/location, name, and occurrence fields null. With no derived spatial store or creation path, Private Home cannot receive a default discovery point and erasure has no spatial record to clean up. Tests remain skipped, so ELP-730 is not checked. Next code task: ELP-530.
- Persistence/authority evidence: Domain EventLocationPrivacy 63/63, repository 12/12, relational model 1/1, retained-authority PostgreSQL 16/16, Clean Architecture 15/15, and final independent persistence/authority re-reviews PASS. ELP-230A database migration enforcement is now complete; ELP-230B backfill acceptance is complete; OREA-210 owns the still-open conditional startup replay correction.
