...
| PublicIngestion | Anonymous machine/browser-submitted telemetry or relay payload. | Abuse controls, payload validation, tenant/host binding, dedicated rate-limit policy. | `[AllowAnonymous]` only when explicitly intended. | `RuntimeTenantOptionalPublicRead` or endpoint-specific tenant binding. | `AnalyticsRelay` or equivalent dedicated limiter. | No output cache. |
