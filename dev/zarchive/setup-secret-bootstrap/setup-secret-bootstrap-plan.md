...
#### ~~Task 1.5: Startup Logging (Blazor BFF)~~ REMOVED

> **Architectural correction:** The Blazor BFF is a separate deployable that communicates with the API only via HTTP/YARP. It has NO access to Application/Infrastructure layers. The secret is only logged by the API. The BFF tells users: "Check the API server logs."
...
> **Architectural correction:** The Blazor BFF is a separate deployable that has NO access to Application/Infrastructure. Secret validation is an API-only concern. The BFF calls this endpoint through YARP like any other API call.
