...
namespace Explore.Application.Features.Events.Handlers.Commands
{
    ...
    public class DeleteEventCommandHandler : IRequestHandler<DeleteEventCommand, bool>
    {
        ...
        private async Task<bool> IsSystemAdmin(Guid userId, CancellationToken cancellationToken)
        {
            try
            {
                // Get all UserRoles with MasterCode="Admin"
                var adminRoles = await _userRoleRepository.GetAll();
                var adminRole = adminRoles.FirstOrDefault(r =>
                    r.MasterCode != null &&
                    r.MasterCode.Equals("Admin", StringComparison.OrdinalIgnoreCase));

                if (adminRole == null)
                {
                    _logger.LogWarning("Admin role not found in UserRole table");
                    return false;
                }

                // Check if user has this role in any tenant
                var tenantUsers = await _tenantUserRepository.GetAll();
                var userTenantRole = tenantUsers.FirstOrDefault(tu =>
                    tu.UserId == userId &&
                    tu.UserRoleId == adminRole.Id);

                return userTenantRole != null;
            }
            catch (Exception ex)
            {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                var members = await _organizationMemberRepository.GetAll();
                var member = members.FirstOrDefault(m =>
                    m.OrganizationId == organizationId &&
                    m.UserId == userId &&
                    !m.IsDeleted); // Ensure member is not soft-deleted
