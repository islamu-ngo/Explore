...
internal static class ContextSystemHelpers
{
    ...
    private static readonly HashSet<string> TestProjects = new(StringComparer.OrdinalIgnoreCase)
    {
        "Event.Application.UnitTests", "Event.Domain.UnitTests", "Event.Architecture.Tests",
        "Event.Persistence.IntegrationTests", "Event.API.IntegrationTests", "Explore.Blazor.Client.Tests",
        "Explore.Blazor.Client.E2ETests", "Explore.Blazor.IntegrationTests", "Explore.Secrets.UnitTests",
        "Explore.Infrastructure.Tests", "Explore.Diagnostic.UnitTests", "Event.Benchmarks"
    };
    ...
    public static string RepoPath(params string[] segments)
    {
        if (segments.Length > 0)
        {
            ...
            if (TestProjects.Contains(first))
            {
                return Path.Combine(new[] { RepoRoot, "tests" }.Concat(segments).ToArray());
            }
        }
