...
// ABOUTME: Test-based generator reads schemas/openapi_islamu-event.json and writes the committed docs artifact.
...
public sealed class ApiContractInventoryGeneratorTests
{
    ...
    public async Task ApiContractInventory_Generate_WritesMarkdownToDocs()
    {
        ...
        await Assert.That(File.Exists(schemaPath)).IsTrue()
            .Because("schemas/openapi_islamu-event.json is the committed build-time contract used by generated clients.");
