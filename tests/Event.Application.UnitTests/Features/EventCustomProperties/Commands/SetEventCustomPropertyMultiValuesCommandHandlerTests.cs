...
public class SetEventCustomPropertyMultiValuesCommandHandlerTests
{
    ...
    public async Task Handle_WhenMultiValueRowQuotaExceeded_ReturnsFailure()
    {
        ...
        await Assert.That(result.QuotaExceeded.Scope).IsEqualTo("event_custom_property_multi_values");
    ...
    public async Task Handle_WhenMultiValueRowCountEqualsQuota_Succeeds()
    {
        ...
        await _repository.Received(1).SetMultiValues(
            _definitionId,
            _eventId,
            Arg.Is<IReadOnlyCollection<EventCustomPropertyValue>>(values => values.Count == 3),
