// ABOUTME: Unit tests for EventTemplateInstantiationService covering template-to-runtime definition creation.
...
public class EventTemplateInstantiationServiceTests
{
    private readonly EventTemplateInstantiationService _service = new();
    private readonly Guid _eventId = Guid.NewGuid();
    private readonly Guid _tenantId = Guid.NewGuid();
    private readonly string _userId = Guid.NewGuid().ToString();

    private EventTemplate CreateTemplate(
        string templateKey = "conference-standard",
        int version = 1,
        params EventTemplateCustomPropertyDefinition[] definitions)
    {
    ...
    private EventTemplateCustomPropertyDefinition CreateTemplateDef(
        Guid? id = null,
        string ns = "tenant.community",
        string key = "notes",
        PropertyType type = PropertyType.Text,
        params EventTemplateCustomPropertyOption[] options)
    {
    ...
    private static EventTemplateCustomPropertyOption CreateTemplateOption(
        Guid defId,
        string ns = "tenant.community",
        string key = "option1",
        string value = "opt1",
        ...
        Guid? parentOptionId = null)
    {
        ...
        {
            Id = Guid.NewGuid(),
            EventTemplateCustomPropertyDefinitionId = defId,
            Namespace = ns,
            Key = key,
    ...
    #region InstantiateFromTemplate

    [Test]
    public async Task InstantiateFromTemplate_WithEmptyTemplate_ReturnsEmptyDefinitions()
    {
        var template = CreateTemplate();

        var result = _service.InstantiateFromTemplate(_eventId, _tenantId, template, _userId);
    ...
    [Test]
    public async Task InstantiateFromTemplate_CopiesDefinitionFields()
    {
        ...
        var template = CreateTemplate();
        // Use reflection to add definition to template's backing list
        var defsField = typeof(EventTemplate).GetField("_definitions", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        var defsList = (List<EventTemplateCustomPropertyDefinition>)defsField!.GetValue(template)!;
        ...
        await Assert.That(runtimeDef.EventId).IsEqualTo(_eventId);
    ...
    [Test]
    public async Task InstantiateFromTemplate_SetsProvenanceFields()
    {
        var templateDef = CreateTemplateDef();
        var template = CreateTemplate(version: 3);
        var defsField = typeof(EventTemplate).GetField("_definitions", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        ((List<EventTemplateCustomPropertyDefinition>)defsField!.GetValue(template)!).Add(templateDef);
    ...
    [Test]
    public async Task InstantiateFromTemplate_SetsAuditFields()
    {
        var templateDef = CreateTemplateDef();
        var template = CreateTemplate();
        var defsField = typeof(EventTemplate).GetField("_definitions", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        ...
        var result = _service.InstantiateFromTemplate(_eventId, _tenantId, template, userId.ToString());
        var runtimeDef = result.Definitions[0].Definition;
    ...
    [Test]
    public async Task InstantiateFromTemplate_CopiesOptionsWithNewIds()
    {
        ...
        var optsField = typeof(EventTemplateCustomPropertyDefinition).GetField("_options", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        var optsList = (List<EventTemplateCustomPropertyOption>)optsField!.GetValue(templateDef)!;
    ...
    [Test]
    public async Task InstantiateFromTemplate_SetsOptionProvenance()
    {
        var templateDef = CreateTemplateDef(type: PropertyType.Option);
        var opt = CreateTemplateOption(templateDef.Id);
        var optsField = typeof(EventTemplateCustomPropertyDefinition).GetField("_options", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        ((List<EventTemplateCustomPropertyOption>)optsField!.GetValue(templateDef)!).Add(opt);

        var template = CreateTemplate(version: 2);
        var defsField = typeof(EventTemplate).GetField("_definitions", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        ((List<EventTemplateCustomPropertyDefinition>)defsField!.GetValue(template)!).Add(templateDef);
    ...
    [Test]
    public async Task InstantiateFromTemplate_RemapsDefaultOptionId()
    {
        var templateDef = CreateTemplateDef(type: PropertyType.Option);
    ...
    [Test]
    public async Task InstantiateFromTemplate_RemapsParentOptionId()
    {
        var templateDef = CreateTemplateDef(type: PropertyType.Option);
    ...
    [Test]
    public async Task InstantiateFromTemplate_CreatesDefaultValueForText()
    {
        ...
        await Assert.That(defaultValue.EventId).IsEqualTo(_eventId);
        await Assert.That(defaultValue.TenantId).IsEqualTo(_tenantId);
        await Assert.That(defaultValue.Ordinal).IsEqualTo(0);
    }

    [Test]
    public async Task InstantiateFromTemplate_CreatesDefaultValueForNumber()
    {
    ...
    [Test]
    public async Task InstantiateFromTemplate_NoDefaultValue_WhenNoDefaultsSet()
    {
        var templateDef = CreateTemplateDef(type: PropertyType.Text);
    ...
    [Test]
    public async Task InstantiateFromTemplate_MultipleDefinitions_AllCopied()
    {
    ...
    [Test]
    public async Task InstantiateFromTemplate_NormalizesNamespaceAndKey()
    {
    ...
    public async Task MatchByProvenance_WithSourceIdMatch_ReturnsSourceIdMatchType()
    {
        ...
        var existing = new EventCustomPropertyDefinition
        {
            Id = Guid.NewGuid(),
            EventId = _eventId,
            TenantId = _tenantId,
    ...
    public async Task MatchByProvenance_MixedMatches_SourceIdAndNamespaceKey()
    {
        ...
        var existing1 = new EventCustomPropertyDefinition
        {
            Id = Guid.NewGuid(),
            EventId = _eventId,
            TenantId = _tenantId,
        ...
        var existing2 = new EventCustomPropertyDefinition
        {
            Id = Guid.NewGuid(),
            EventId = _eventId,
            TenantId = _tenantId,
