...
public sealed class ConfigurationManifestExportSectionTests : IDisposable
{
    ...
    [Test]
    public async Task SourcePlacementUsesInstanceNavigationAndRtlSafeScopedStyles()
    {
        ...
        var layout = await File.ReadAllTextAsync(Path.Combine(
            ...
            "Instance",
            "Components",
            "InstanceAdminSettingsLayout.razor"));
        var tenantLayout = await File.ReadAllTextAsync(Path.Combine(
            repositoryRoot,
            "src",
            "Explore.Blazor.Client",
            "Pages",
            ...
            "Tenant",
            "Components",
            "TenantAdminSettingsLayout.razor"));
        ...
        var component = await File.ReadAllTextAsync(Path.Combine(
            repositoryRoot,
            "src",
            "Explore.Blazor.Client",
            "Pages",
        ...
        await Assert.That(layout).Contains("<ConfigurationManifestExportSection");
        await Assert.That(tenantLayout).DoesNotContain("ConfigurationManifest");
        await Assert.That(component).DoesNotContain("ClaimsPrincipal");
        await Assert.That(component).DoesNotContain("IsInRole");
        await Assert.That(component).DoesNotContain("/api/control-plane/configuration-manifest/export");
        await Assert.That(css).Contains("inline");
