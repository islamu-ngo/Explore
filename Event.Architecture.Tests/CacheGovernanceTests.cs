...
public sealed partial class CacheGovernanceTests
{
    ...
    {
        var cachingExtensions = File.ReadAllText(Path.Combine(SourceRoot, "Explore.API", "Extensions", "CachingExtensions.cs"));
    ...
    public async Task TenantScopedEventLists_ShouldUseTenantNamespaceAndTenantInvalidationTag()
    {
        ...
        var cacheTags = File.ReadAllText(Path.Combine(SourceRoot, "Explore.Application", "Caching", "CacheTags.cs"));
    ...
    {
        var applicationRoot = Path.Combine(SourceRoot, "Explore.Application");
