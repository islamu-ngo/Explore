...
public class OrganizationCentricGuardrailTests
{
    ...
    {
        var domainDirectory = RepositoryRoot.GetDirectories("Explore.Domain").Single();
    ...
    private static DirectoryInfo RepositoryRoot
    {
        get
        {
            ...
            while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "Explore.sln")))
            {
                directory = directory.Parent;
            }

            return directory ?? throw new DirectoryNotFoundException("Could not find repository root containing Explore.sln.");
        }
    }
