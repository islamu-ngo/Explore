...
public sealed class CheckoutDispatchEffect : ITenantEntity, IAuditableEntity
{
    public const int MaxLeaseOwnerLength = 200;
    public const int MaxFailureCodeLength = 120;

    private CheckoutDispatchEffect()
    {
    }
    ...
    public static CheckoutDispatchEffect Create(PaymentAttempt attempt, DateTime createdAt)
    {
    ...
    public void Claim(string leaseOwner, Guid leaseToken, DateTime leaseExpiresAt, DateTime claimedAt)
    {
        ...
        {
            throw new InvalidOperationException("Checkout dispatch effect is not claimable.");
        }
    ...
    public void RecoverExpiredClaim(DateTime recoveredAt)
    {
        ...
        {
            throw new InvalidOperationException("Only an expired checkout dispatch claim can be recovered.");
        }
