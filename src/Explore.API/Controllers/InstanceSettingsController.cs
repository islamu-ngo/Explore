...
public class InstanceSettingsController : ExploreControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        var userId = await ResolveCurrentUserIdAsync(_mediator, cancellationToken);
        if (!userId.HasValue) return this.ToAuthenticationRequiredProblem(detail: "The authenticated principal could not be resolved to an application user.");
        if (!await _adminContext.IsInstanceAdminAsync(userId.Value, cancellationToken)) return this.ToForbiddenProblem(detail: "Instance administrator authority is required for this operation.");
