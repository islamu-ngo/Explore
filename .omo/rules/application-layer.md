...
- **Record Requests**: Follow the [canonical record-selection policy](../../docs/GOVERNANCE.md#canonical-record-selection-policy). Concrete handwritten MediatR requests default to sealed records; choose positional versus nominal form for construction safety. `BaseCommandResponse<T>` remains a class. Current `UserId`/`TenantId` never comes from a body; a legitimate target ID still requires server authorization.
