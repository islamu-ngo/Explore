<!-- ABOUTME: Pattern and resolution workflow for cross-space links in multi-space sites. -->
<!-- ABOUTME: Explains https://app.gitbook.com/s/<spaceId>/<path> syntax, XSPACE_ sentinels, and replacement scripts. -->
...
## Note on content authoring

[content-authoring.md](content-authoring.md) covers the markdown syntax for links in general. It should be aware that **cross-space links require resolved space IDs and won't work as relative paths**, and should produce them using the sentinel pattern when generating content for a multi-space site being scaffolded. The actual sentinel→ID resolution belongs to [site-configuration.md](site-configuration.md), since this skill is the one orchestrating space creation and has the IDs as soon as they exist.

When authoring content where the target space already exists and the target space already exists, it doesn't need a sentinel at all — it can fetch the ID and path directly via the API (`GET /orgs/{orgId}/spaces`, `GET /spaces/{spaceId}/content/pages`), same as the "Linking to a space that already exists" section above. Sentinels are only needed when the target space doesn't exist yet.