<!-- ABOUTME: ContentKit UI component reference for GitBook platform integrations. -->
<!-- ABOUTME: Covers JSX layout containers, cards, interactive inputs, buttons, and state action reducers. -->
...
To make a block round-trip through Git Sync as a readable code fence instead of an opaque node, add `markdown: { codeblock, body }` to its manifest entry — see [integration-manifest.md](integration-manifest.md). The native Mermaid block uses this pattern.
