<!-- ABOUTME: Playbook for migrating documentation from Mintlify, Docusaurus, ReadTheDocs, or GitBook v1. -->
<!-- ABOUTME: Outlines pre-flight checks, anchor-page rebuilds, format sweeps, and link normalization. -->
...
| `icon={<svg>...</svg>}` (inline SVG in a prop) | Convert the SVG into either a Font Awesome icon name (where one exists) or a separate file under `.gitbook/assets/` referenced via `<img>` — see [markdown.md](markdown.md) for SVG handling |
