<!-- ABOUTME: Space configuration reference for .gitbook.yaml, SUMMARY.md, and .gitbook/ layout. -->
<!-- ABOUTME: Details redirects, space variables, includes, tags, and table of contents grammar. -->
...
* **Site-wide Git Sync (default)** — the site's "Project directory" points to where `gitbook-docs.yaml` lives, not to any individual space's `.gitbook.yaml`. Each space's directory (containing its own `.gitbook.yaml`) is set separately via `gitbook-docs.yaml`'s content mapping. See [git-sync-handoff.md](git-sync-handoff.md).
