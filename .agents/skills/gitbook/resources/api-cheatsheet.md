<!-- ABOUTME: Quick reference cheatsheet for the GitBook v1 REST API. -->
<!-- ABOUTME: Lists endpoints, request/response bodies, and the gbapi curl helper for sites and spaces. -->
...
This cheatsheet is scoped to what site and space orchestration actually needs. The full API surface is much larger — for less common endpoints, search the GitBook docs directly.
...
For each `site-section`, the `siteSpaces` array contains one entry per language. Only the language that originated from Git has a populated `gitSync` field on its space; the rest are auto-translated and live entirely in GitBook. See `example-site/structure.json` for a complete real-world example.
