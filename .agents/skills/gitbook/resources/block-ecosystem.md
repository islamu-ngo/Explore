<!-- ABOUTME: Decision matrix and selection heuristics for GitBook specialized blocks. -->
<!-- ABOUTME: Helps choose the right rich block (tabs, steppers, hints, card-tables, updates) over plain prose. -->
...
[blocks.md](blocks.md) and [content-authoring.md](content-authoring.md) provide the syntax of each block. This file is about *which block to use when*, with example invocations to anchor each pattern.
