<!-- ABOUTME: Full specification for GitBook x-* OpenAPI extensions and operation lifecycle tags. -->
<!-- ABOUTME: Documents x-page-title, x-page-icon, x-parent, x-enable-proxy, code samples, and stability. -->
...
For making the interactive runner work as a whole (declaring auth, servers, CORS, proxy together), see [openapi-test-it.md](openapi-test-it.md).
...
Route "Test it" requests through GitBook's server-side proxy, which bypasses CORS restrictions on the target API. Root applies to every operation; an operation-level value overrides the root. The proxy only forwards to URLs in the `servers` array. Full treatment in [openapi-test-it.md](openapi-test-it.md).
