...
Load this skill for text-to-Lottie, Bodymovin JSON, SVG/logo/type animation,
loaders/icons, UI microinteractions, lower thirds, diagrams, data/stat/chart
animations, product promos, scene motion, visual effects, scene edits,
slots/controls, and Skottie debugging.

## When NOT to Load

Do not load this for static raster image generation, generic SVG editing without
animation output, CSS-only web motion, or video editing outside a Lottie/Skottie
deliverable.

## Must-Read Docs

Always read `references/player-contract.md` before creating, editing, fixing, or
verifying a scene. Then load only the routed references from the table below.
...
- Switching away from the official Skia Skottie player to make a scene pass.
- Overwriting a non-placeholder `main-project/scene-1` without an explicit target.
- Adding cards, borders, dividers, shadows, glow, or stacked tints to make a
  design seem premium when scale, spacing, weight, brightness, and timing can do
  the job.
- Falling back to one uniform linear ease for every layer.
- Finishing after JSON validation without frame inspection in the player.

## Minimal Examples

Route a logo animation prompt to `references/player-contract.md`,
`references/recipe-logo.md`, `references/motion-taste.md`, and
`references/design-taste.md`; resolve the target scene; edit the scene
`lottie.json`; validate JSON; inspect frame `0`, midpoint, and `op - 1`.
