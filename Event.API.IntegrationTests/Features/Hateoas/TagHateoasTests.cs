...
/// <summary>
/// HATEOAS-specific tests for TagController.
/// Validates tag-specific links including events using this tag.
/// </summary>
[ClassDataSource<ApiTestFixture>(Shared = SharedType.PerAssembly)]
public class TagHateoasTests
{
    private readonly ApiTestFixture _fixture;
    private const string BaseUrl = "/api/v1/tag";

    public TagHateoasTests(ApiTestFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task GetAll_ItemLinks_ShouldIncludeSelfLink()
    {
            ...
            items.GetArrayLength() > 0)
        {
            ...
            if (firstItem.TryGetProperty("_links", out var itemLinks))
            {
                ...
                await Assert.That(href).Contains("/api/v1/tag/");
            }
        }
    }
    ...
    public async Task GetAll_ItemLinks_ShouldIncludeEventsLink()
    {
            ...
            items.GetArrayLength() > 0)
        {
            ...
            if (firstItem.TryGetProperty("_links", out var itemLinks))
            {
                // Tag items should have events link (events with this tag)
    ...
    public async Task GetAll_ItemLinks_ShouldIncludeCollectionLink()
    {
            ...
            items.GetArrayLength() > 0)
        {
            ...
            if (firstItem.TryGetProperty("_links", out var itemLinks))
            {
                ...
                await Assert.That(href).IsEqualTo("/api/v1/tag");
            }
        }
    }
    ...
    {
        // Arrange - First get list to find a tag
        ...
        // Act
        var response = await _fixture.Client.GetAsync($"{BaseUrl}/{tagId}");
